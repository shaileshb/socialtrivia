...
public class Dictionary {
    ...
    for (String fname : fnames) {
      try {
        ...
        while (true) {
          ...
          if (tokens.length != 2) {
            log.warning("Could not parse line: " + str + ", tokens.length = "
                + tokens.length);
            continue;
          }
  ...
  private HashSet<String> mutate1(String str, HashSet<String> mset,
    ...
    for (int i = 0; i < str.length(); i++) {
      ...
      for (char c = 'a'; c <= 'z'; c++) {
        addWord(str.substring(0, i) + c + str.substring(i + 1), mset, keepAll); // substitute
        addWord(str.substring(0, i) + c + str.substring(i), mset, keepAll); // insert
      }
    }
    for (char c = 'a'; c <= 'z'; c++) {
